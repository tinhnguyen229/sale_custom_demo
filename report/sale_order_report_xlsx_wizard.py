...
class SaleOrderReportXlsxWizard(models.AbstractModel):
    # _name = 'report.module_name.report_name'
    _name = 'report.sale_custom_demo.report_saleorder_xlsx_test_wizard'
    _description = "Report Sale Order Xlsx Wizard"
    ...
    def generate_data(self, workbook, sheet, objs):
        print(objs)
        obj = objs[0]
        print(obj.user_id.name)
        print('cmkcmsmkc')
        sheet.write(0, 7, obj.user_id.name)
        sheet.write(0, 8, obj.date_from.strftime('%Y-%m-%d'))
        sheet.write(0, 9, obj.date_to.strftime('%Y-%m-%d'))
